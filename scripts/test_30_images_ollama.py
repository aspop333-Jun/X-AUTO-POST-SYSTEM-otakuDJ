...
"""
    ...
    try:
        ...
        try:
            ...
            # JSON部分を抽出
            import re
            match = re.search(r'\{[^{}]*"criteria"[^{}]*\{[^}]+\}[^}]*\}', content, re.DOTALL)
            if match:
                try:
...
def main():
    print("=" * 80)
    print("🐯 Kotaro Scoring V4.6.1 - 30枚画像テスト (Ollama)")
    print("=" * 80)
