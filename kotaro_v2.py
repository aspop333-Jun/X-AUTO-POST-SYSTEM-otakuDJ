"""
[DEPRECATED] Kotaro-Engine v2: 画像解析 → 18文字生成

⚠️ このファイルは非推奨です。kotaro_api.py を使用してください。
⚠️ 現在のシステムはQwen (Ollama) を使用しています。

旧: llava + gemma3:4b
新: qwen2.5vl:7b + qwen2.5:7b-instruct-q4_K_M
"""

import ollama
import random
import base64
from pathlib import Path


# =============================================================================
# 表情カテゴリ
# =============================================================================

EXPRESSION_CATEGORIES = [
    "笑顔",      # 笑っている
    "クール",    # すましている、かっこいい
    "かわいい",  # かわいらしい表情
    "ふざけ",    # ふざけている、おどけている
    "真剣",      # 真剣な表情
]

# =============================================================================
# 画像解析（llava）
# =============================================================================

def analyze_photo(image_path: str) -> str:
    """llavaで写真の表情を解析"""
    
    # 画像をbase64エンコード
    image_data = Path(image_path).read_bytes()
    
    # llavaに表情を聞く
    response = ollama.generate(
        model="llava",
        prompt="""この写真の人物の表情を1つだけ選んでください：
- 笑顔（笑っている）
- クール（すまし顔、かっこいい）
- かわいい（可愛らしい表情）
- ふざけ（おどけている）
- 真剣（真剣な表情）

表情の名前だけ答えて：""",
        images=[image_data],
    )
    
    result = response["response"].strip()
    
    # カテゴリにマッチするか確認
    for cat in EXPRESSION_CATEGORIES:
        if cat in result:
            return cat
    
    return "笑顔"  # デフォルト


# =============================================================================
# Kotaro-Engine（18文字生成）
# =============================================================================

# 表情別のお手本
EXAMPLES = {
    "笑顔": [
        "栞さんマジ可愛い、優勝✨",
        "栞さんの笑顔、これは神回📸",
        "栞さん撮れたの最高すぎる✨",
        "栞さんいい笑顔もらった📸",
    ],
    "クール": [
        "栞さんカッコよすぎ、痺れる✨",
        "栞さんのこの表情、ヤバい📸",
        "栞さんクールすぎて優勝✨",
    ],
    "かわいい": [
        "栞さん可愛い、これは反則✨",
        "栞さんの可愛さ、規格外📸",
        "栞さん可愛すぎて無理✨",
    ],
    "ふざけ": [
        "栞さんのこのノリ、最高📸",
        "栞さん面白すぎる✨",
        "栞さんこれ好き、優勝📸",
    ],
    "真剣": [
        "栞さんの真剣な眼差し、刺さる✨",
        "栞さんこの表情、美しい📸",
        "栞さんの集中力、ヤバい✨",
    ],
}


def generate_comment(model_name: str, expression: str) -> str:
    """表情に合わせた18文字コメントを生成"""
    
    examples = EXAMPLES.get(expression, EXAMPLES["笑顔"])
    few_shot = "\n".join([f"- {ex.replace('栞', model_name)}" for ex in examples])
    
    prompt = f"""虎太郎として{model_name}さんの写真に一言。
虎太郎＝30代後半、落ち着いた大人。ノリはいいけど若者スラングは使わない。

写真の表情: {expression}

【お手本】
{few_shot}

【禁止】卍、ぴえん、草、w、若者スラング

【出力】18文字以内で1つだけ："""
    
    response = ollama.generate(
        model="gemma3:4b",
        prompt=prompt,
        options={"temperature": 0.8, "num_predict": 50}
    )
    
    result = response["response"].strip()
    
    # 改行があれば最初の行だけ
    if "\n" in result:
        result = result.split("\n")[0]
    
    # 卍が含まれていたら削除
    result = result.replace("卍", "")
    
    # 18文字に切り詰め
    if len(result) > 18:
        result = result[:17] + "✨"
    
    return result


# =============================================================================
# メイン
# =============================================================================

def main():
    import argparse
    
    parser = argparse.ArgumentParser(description="Kotaro-Engine v2")
    parser.add_argument("--image", type=str, required=True, help="写真のパス")
    parser.add_argument("--name", type=str, default="栞", help="モデルさんの名前")
    parser.add_argument("--count", type=int, default=3, help="生成数")
    
    args = parser.parse_args()
    
    print("\n" + "=" * 50)
    print("🐯 Kotaro-Engine v2.0 (Vision)")
    print("=" * 50)
    
    # 写真解析
    print(f"\n📸 写真: {args.image}")
    print("🔍 表情を解析中...")
    
    expression = analyze_photo(args.image)
    print(f"✅ 表情: {expression}")
    
    # 18文字生成
    print(f"\n🐯 {args.name}さんへの18文字コメント:")
    print("-" * 40)
    
    for i in range(args.count):
        comment = generate_comment(args.name, expression)
        print(f"  [{i+1}] {comment} ({len(comment)}文字)")
    
    print("-" * 40)
    print("=" * 50 + "\n")


if __name__ == "__main__":
    main()
